<div class="col-md-12 text-center">
    ...
    @php
        ...
        $existenTestigos = $testigos->isNotEmpty();
        $array_testigos = array();
        if($existenTestigos){
            foreach ($testigos as $testigo) {
                if(empty($testigo->nombre)){
                    $data = ["idTestigo"=>$testigo->id,"nombreTestigo" => "","paternoTestigo" => "","maternoTestigo" => "","adicionalTestigo" => $testigo->descripcion_involucrado];
                }else{
                    $data = ["idTestigo"=>$testigo->id,"nombreTestigo" => $testigo->nombre,"paternoTestigo" => $testigo->primer_apellido,"maternoTestigo" => $testigo->segundo_apellido,"adicionalTestigo" => $testigo->descripcion_involucrado];
                }
                array_push($array_testigos,$data);
            }
            $arrayTestigos = json_encode($array_testigos);
        }
@endphp
    <div>
    ...
    <div class="custom-control custom-radio custom-control-inline">
        <input
          ...
          @if ($existenTestigos){{'checked'}}@endif>
...
<div id="testigosDiv" class="container pl-3">
    ...
    <div id="DivTestigos" class="@if (!$existenTestigos){{'d-none'}}@endif">
